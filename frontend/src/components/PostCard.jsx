...
const PostCard = ({ post, isAuthorView, onDelete, onComplete, onProgressChange }) => {
  ...
  return (
    <>
      ...
      <div className="p-4 sm:p-5 rounded-2xl bg-[#1A3D63]/85 border border-[#1A3D63]/40 hover:border-[#1A3D63]/80 transition-all duration-300 backdrop-blur-xl shadow-xl group">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex-1 min-w-0 text-left">
            ...
            <div className="flex items-center gap-3 mb-1.5 flex-wrap">
              ...
              {post.status === 'completed' && (
                <span className="px-2 py-0.5 rounded-md bg-[#0A1931] text-[#B3CFE5] border border-[#1A3D63]/50 text-[10px] font-mono font-bold flex items-center gap-1 shadow-sm">
            ...
            {post.skills && Array.isArray(post.skills) && post.skills.length > 0 && (
              <div className="flex flex-wrap gap-1.5 items-center">
                ...
                {post.skills.map((skill) => (
                  <span
                    key={skill}
                    className="px-2 py-0.5 rounded-md bg-[#0A1931]/80 border border-[#1A3D63]/40 text-[#B3CFE5] text-[11px] font-mono"
                  >
                    {skill}
                  </span>
                ))}
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            {mediaUrl && (
              <div className="w-18 h-18 sm:w-20 sm:h-20 rounded-xl overflow-hidden border border-[#1A3D63]/40 bg-[#0A1931] shrink-0 hidden sm:block">
                ...
                <div className="flex items-center gap-1.5">
                  {post.status === 'live' && onComplete && (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-[#0A1931] hover:bg-[#1A3D63] border border-[#1A3D63]/40 text-[#B3CFE5] font-semibold text-xs transition-colors"
                    >
        ...
        <div className="mt-4 pt-3 border-t border-[#1A3D63]/30">
