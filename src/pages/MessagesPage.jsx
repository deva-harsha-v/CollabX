...
const ChatPanel = ({ room, currentUser, onBack }) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel("msg_page_" + room.room_id)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "chat_messages", filter: "chat_room_id=eq." + room.room_id },
        async (payload) => {
          const newMsg = payload.new;
          const { data: prof } = await supabase.from("profiles").select("name, avatar_url").eq("id", newMsg.sender_id).single();
          const formattedMsg = { ...newMsg, profiles: prof || { name: "User", avatar_url: "" } };
  ...
  const handleSend = async (e) => {
    ...
    setInputText(""); setAttachment(null); setAttachmentName("");
    const { data: newMsg, error: sendErr } = await sendChatMessage(room.room_id, content, attach);
    if (sendErr) { setChatError("Failed to send: " + (sendErr.message || "Permission denied")); setInputText(content); return; }
    if (newMsg) { setMessages(prev => prev.some(m => m.id === newMsg.id) ? prev : [...prev, newMsg]); setTimeout(scrollToBottom, 100); }
  };
  ...
  if (!room) return (
    <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
      ...
      <p className="text-sm text-[#B3CFE5]/70 max-w-xs">Pick a challenge from the left panel to open its collaboration chat.</p>
    </div>
  );
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      <div className="px-5 py-4 border-b border-[#4A7FA7]/30 bg-[#0A1931]/80 flex items-center gap-3 shrink-0">
        <button onClick={onBack} className="md:hidden p-1.5 rounded-lg bg-[#1A3D63] border border-[#4A7FA7]/40 text-[#B3CFE5] hover:text-[#F6FAFD] transition-colors">
      ...
      <div className="flex-1 p-4 sm:p-6 overflow-y-auto space-y-4 min-h-0">
        {loading ? (
          <div className="py-12 text-center text-[#B3CFE5] font-mono text-xs flex items-center justify-center gap-2">
            <span className="w-2 h-2 rounded-full bg-[#B3CFE5] animate-ping" /><span>Loading chat history...</span>
          </div>
          ...
          return (
            <div key={msg.id} className={"flex items-start gap-2.5 " + (isMine ? "flex-row-reverse" : "flex-row")}>
              ...
              <div className={"max-w-[75%] space-y-1 " + (isMine ? "text-right" : "text-left")}>
                ...
                <div className={"p-3 rounded-2xl text-xs leading-relaxed " + (isMine
                  ...
                  {msg.attachment_url && (
                    <div className="mt-2 pt-2 border-t border-white/20">
                      {msg.attachment_type === "image"
                        ...
                        : <a href={msg.attachment_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 underline text-[#B3CFE5] hover:text-[#F6FAFD] font-mono text-[11px]"><FileText className="w-3.5 h-3.5" /><span>View Attachment</span></a>}
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="py-16 text-center text-[#B3CFE5]/70 font-mono text-xs">No messages yet. Start the collaboration!</div>
        )}
      ...
      {attachmentName && (
        <div className="px-4 py-2 bg-[#0A1931] border-t border-[#4A7FA7]/30 flex items-center justify-between text-xs text-[#B3CFE5] font-mono">
          <span className="flex items-center gap-1.5 truncate"><Paperclip className="w-3.5 h-3.5" /> {attachmentName}</span>
          <button type="button" onClick={() => { setAttachment(null); setAttachmentName(""); }} className="text-[#B3CFE5]/70 hover:text-red-400 ml-2"><X className="w-3.5 h-3.5" /></button>
        </div>
      )}
      <form onSubmit={handleSend} className="p-3 sm:p-4 bg-[#0A1931] border-t border-[#4A7FA7]/30 flex items-center gap-2 shrink-0">
        <label className="p-2.5 rounded-xl bg-[#1A3D63] hover:bg-[#244b78] text-[#B3CFE5] hover:text-[#F6FAFD] cursor-pointer transition-colors shrink-0 border border-[#4A7FA7]/30">
          ...
          <input type="file" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) { setAttachment(f); setAttachmentName(f.name); } }} />
        </label>
        ...
        <button type="submit" disabled={!inputText.trim() && !attachment} className="red-pill-button px-5 py-2.5 text-xs font-bold shadow-md disabled:opacity-40 shrink-0">
          <span>Send</span><Send className="w-3.5 h-3.5 ml-1 inline" />
        </button>
      </form>
    </div>
  );
...
const MessagesPage = () => {
  ...
  const handleSelectRoom = (room) => { setSelectedRoom(room); setShowChat(true); };
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 pt-24 pb-8" style={{ height: "calc(100vh - 0px)" }}>
        <div className="h-full flex flex-col">
          <div className="mb-4 shrink-0">
            ...
            <h1 className="font-[Outfit] font-extrabold text-2xl sm:text-3xl text-[#F6FAFD] tracking-tight">Your Collaboration Rooms</h1>
            <p className="text-sm text-[#B3CFE5] mt-1">All challenge chat rooms you have access to as a poster or accepted solver.</p>
          </div>
          <div className="flex-1 min-h-0 flex rounded-2xl border border-[#4A7FA7]/30 overflow-hidden bg-[#1A3D63]/60 backdrop-blur-xl shadow-2xl" style={{ maxHeight: "calc(100vh - 11rem)" }}>
            <div className={"w-full md:w-72 lg:w-80 shrink-0 flex flex-col border-r border-[#4A7FA7]/30 bg-[#0A1931]/60 " + (showChat ? "hidden md:flex" : "flex")}>
              <div className="px-4 py-3 border-b border-[#4A7FA7]/30 shrink-0">
                <span className="text-xs font-mono font-bold text-[#B3CFE5] uppercase tracking-wider">Active Rooms ({rooms.length})</span>
              </div>
              <div className="flex-1 overflow-y-auto">
                {loadingRooms ? (
                  <div className="py-12 text-center text-[#B3CFE5]/70 font-mono text-xs flex flex-col items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-[#B3CFE5] animate-ping" />Loading rooms...
                  </div>
                ) : rooms.length === 0 ? (
                  <div className="py-12 px-4 text-center text-[#B3CFE5]/70 font-mono text-xs">
                    ...
                    <p className="text-[#B3CFE5]/60 leading-relaxed text-[11px]">Rooms appear here once a poster accepts your contact request, or when you accept a solver's request.</p>
                  </div>
                ) : rooms.map((room) => {
                  const isActive = selectedRoom?.room_id === room.room_id;
                  return (
                    <button key={room.room_id} type="button" onClick={() => handleSelectRoom(room)}
                      className={"w-full text-left px-4 py-3.5 flex items-center gap-3 transition-all border-b border-[#4A7FA7]/20 last:border-0 " + (isActive ? "bg-[#4A7FA7]/25 border-l-2 border-l-[#B3CFE5]" : "hover:bg-[#1A3D63]/60 border-l-2 border-l-transparent")}>
                      ...
                      <ChevronRight className={"w-4 h-4 shrink-0 transition-colors " + (isActive ? "text-[#B3CFE5]" : "text-[#4A7FA7]/50")} />
                    </button>
                  );
                })}
              </div>
