    ...
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-md animate-fade-in">
        <div 
          ...
          className="relative w-[95vw] sm:w-full max-w-3xl max-h-[90vh] p-5 sm:p-8 bg-[#221226] border border-[#935073]/40 rounded-3xl shadow-[0_0_60px_rgba(147,80,115,0.25)] text-[#F8F4E9] overflow-y-auto"
        >
          ...
          <button
            onClick={onClose}
            className="sticky top-0 float-right z-30 p-2 text-[#F8F4E9]/60 hover:text-[#F8F4E9] rounded-full bg-[#3a1e3e]/80 backdrop-blur-md border border-[#935073]/40 transition-colors"
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              {/* Unlocked Banner for Accepted Solvers */}
              {post.is_authorized && contactStatus === 'accepted' && (
                <div className="p-6 rounded-2xl bg-gradient-to-r from-[#502D55] via-[#3a1e3e] to-[#221226] border-2 border-[#F6DBC0]/60 shadow-[0_0_30px_rgba(246,219,192,0.2)] animate-fade-in">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-mono">
                    {post.phone_number && (
                      <div className="p-3 rounded-xl bg-[#221226]/80 border border-[#935073]/40 flex items-center gap-2.5">
                        <Phone className="w-4 h-4 text-[#F6DBC0] shrink-0" />
                        <div>
                          <span className="text-[10px] text-[#F8F4E9]/60 block">Poster Direct Phone</span>
                    ...
                    {post.author_email && (
                      <div className="p-3 rounded-xl bg-[#221226]/80 border border-[#935073]/40 flex items-center gap-2.5">
                        ...
                        <div>
                          <span className="text-[10px] text-[#F8F4E9]/60 block">Poster Verified Email</span>
                    ...
                    {(post.latitude !== null && post.longitude !== null) && (
                      <div className="p-3 rounded-xl bg-[#221226]/80 border border-[#935073]/40 flex items-center gap-2.5 sm:col-span-2">
                        <Navigation className="w-4 h-4 text-[#F6DBC0] shrink-0" />
                        <div>
                          <span className="text-[10px] text-[#F8F4E9]/60 block">Exact Float GPS Coordinates (Unrounded)</span>
                          <span className="text-sm font-bold text-[#F6DBC0]">
              ...
              <div className="flex items-center justify-between gap-4 pb-4 border-b border-[#935073]/30">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full border border-[#935073]/40 overflow-hidden bg-[#3a1e3e] shrink-0">
                    <img
                  ...
                  <div>
                    <h4 className="text-base font-bold text-[#F8F4E9] flex items-center gap-2">
                      ...
                      <span className="inline-flex items-center gap-0.5 text-[10px] font-mono font-bold text-[#F6DBC0] bg-[#502D55] px-2 py-0.5 rounded-full border border-[#F6DBC0]/30">
                ...
                <div className="text-right text-xs text-[#F8F4E9]/60 font-mono">
                  ...
                  <span className="px-2 py-0.5 rounded bg-[#502D55] text-[#F6DBC0] border border-[#F6DBC0]/40 text-[10px] font-bold uppercase">
                    STATUS: {post.status}
                  </span>
                </div>
              </div>
              ...
              {/* Full Description */}
              <div className="prose prose-invert max-w-none text-sm text-[#F8F4E9]/90 leading-relaxed whitespace-pre-line bg-[#3a1e3e]/40 p-5 rounded-2xl border border-[#935073]/30">
                {post.description}
              </div>
              ...
              {post.skills && Array.isArray(post.skills) && post.skills.length > 0 && (
                <div>
                  <span className="text-xs font-mono uppercase tracking-wider text-[#F8F4E9]/60 block mb-2 flex items-center gap-1.5">
              ...
              {post.media_url && (
                <div>
                  <span className="text-xs font-mono uppercase tracking-wider text-[#F8F4E9]/60 block mb-2">
                    Attached Media / Technical Diagram:
                  </span>
                  <div className="rounded-2xl overflow-hidden border border-[#935073]/30 max-h-96 bg-[#3a1e3e]">
              ...
              {post.address && (
                <div className="p-4 rounded-xl bg-[#3a1e3e]/60 border border-[#935073]/30 flex items-center gap-2.5 text-xs text-[#F8F4E9]/80 font-mono">
                  <MapPin className="w-4 h-4 text-[#F6DBC0] shrink-0" />
                  <div>
                    <span className="text-[10px] text-[#F8F4E9]/60 block">General Address / Region</span>
              ...
              {!post.is_authorized && (
                <div className="p-4 rounded-xl bg-[#502D55]/30 border border-[#935073]/30 flex items-center justify-between gap-4 text-xs font-mono text-[#F6DBC0]">
                  ...
                  <span className="text-[10px] bg-[#502D55] px-2 py-1 rounded border border-[#F6DBC0]/40 text-[#F6DBC0] font-bold">
                    RLS PROTECTED
                  </span>
                </div>
              )}
              ...
              {currentUser && contactStatus !== 'author' && (
                <div 
                  className="sticky bottom-0 pt-4 pb-2 bg-gradient-to-t from-[#221226] via-[#221226]/90 to-transparent flex justify-end z-20"
                  style={{ transform: `translateY(-${scrollOffset}px)`, transition: 'transform 0.15s ease-out' }}
                >
                  {contactStatus === 'pending' ? (
                    <div className="px-6 py-3.5 rounded-full bg-[#502D55]/80 border border-[#935073]/50 text-[#F6DBC0] font-mono text-xs font-bold flex items-center gap-2 shadow-lg">
                      <Clock className="w-4 h-4 text-[#F6DBC0] animate-pulse" />
                      <span>Contact Request Pending Poster Review</span>
                    </div>
                  ) : contactStatus === 'accepted' ? (
                    <div className="px-6 py-3.5 rounded-full bg-[#502D55] border border-[#F6DBC0]/50 text-[#F6DBC0] font-mono text-xs font-bold flex items-center gap-2 shadow-lg">
                      <CheckCircle2 className="w-4 h-4 text-[#F6DBC0]" />
                      <span>Connected — Solver Access Granted</span>
                    </div>
                  ) : contactStatus === 'rejected' ? (
                    <div className="px-6 py-3.5 rounded-full bg-[#3a1e3e] border border-[#935073]/40 text-[#F8F4E9]/60 font-mono text-xs font-medium">
            ...
            <div className="py-12 text-center text-[#F8F4E9]/60 font-mono text-xs">
